...
class ResidualVQ(nn.Module):
    ...
    def forward(
        ...
        should_quantize_dropout = self.training and self.quantize_dropout and not return_loss and random.random() < self.quantize_dropout_prob
...
class GroupedResidualVQ(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.groups = groups
        assert (dim % groups) == 0
        if not accept_image_fmap:
            dim_per_group = dim // groups
        
        if split_dim:
            dim_per_group = kwargs.get('codebook_dim', dim // groups)
        ...
        if self.split_dim is None:
            self.split_dim = 1 if self.accept_image_fmap else -1
